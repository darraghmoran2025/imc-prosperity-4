...
# Position limits per product (updated each round — confirm from round brief)
LIMITS: dict[str, int] = {
    # Round 1
    "EMERALDS": 80,
    "TOMATOES": 80,
    ...
    # Round 3 — limits TBD, confirm from round brief before live submission.
    # Voucher cap 80 matches P3 top team's hedging-headroom sizing.
    "HYDROGEL_PACK": 50,
    "VELVETFRUIT_EXTRACT": 200,
    "VEV_4000": 80, "VEV_4500": 80, "VEV_5000": 80,
    "VEV_5100": 80, "VEV_5200": 80, "VEV_5300": 80,
    "VEV_5400": 80, "VEV_5500": 80, "VEV_6000": 80, "VEV_6500": 80,
}
...
# ============================================================
# ROUND 3 — Velvetfruit Extract vouchers
# ============================================================

# Full set of vouchers observed in data. 6000 and 6500 have mid pinned at 0.5
# (no tradeable edge); 4000 and 4500 are synthetic futures (delta≈1, IV noise).
# The tradeable IV-scalping set is {5000, 5100, 5200, 5300, 5400, 5500}.
...
# VEV TTE: round brief says 7 Solvenarian days starting from day 1.
# Tracked by day_count + timestamp fraction in velvetfruit.tte_years().
VEV_TTE_DAYS_AT_START: int = 7

# IV scalping parameters. Slow EMA (alpha 0.005 ≈ 140-tick half-life) gives a
# stable "fair IV" baseline; observed per-strike σ(IV) across a day ≈ 0.006,
# so short-term deviations from the slow EMA ARE the trade signal.
...
# ────────────────────────────────────────────────────────────────────────
# ──────────────────────────────────────────────────────────────────────────────
# Options math — Black-Scholes + implied vol solver (pure stdlib)
# ──────────────────────────────────────────────────────────────────────────────
...
def implied_vol(price: float, S: float, K: float, T: float, r: float = 0.0) -> float | None:
    ...
    if price <= intrinsic + 1e-6 or T <= 0:
        return None
    lo, hi = 1e-4, 5.0
    # Bisection — ~40 iters for <1e-9 precision on σ∈[0,5]
    for _ in range(40):
        mid = 0.5 * (lo + hi)
...
class HydrogelPackTrader(BaseProductTrader):
    ...
    Tuning history:
      - v1 (simple MM, EMA alpha=0.002, spread=3, rev_thresh=8):
          +34k / 3-day training backtest, BUT at risk on trending days â€”
    ...
    EMA_ALPHA: float = 0.002        # slow long EMA â€” stable fair value
    ...
    # Inventory guardrails â€” only activate when position is genuinely risky.
    # Hidden day failure mode was repeatedly rebuying while already long into a
    # down-leg, then carrying that inventory late. These soft thresholds make
    # HP lean out earlier without shutting the MM down.
    ...
    # Round 3 timestamps run 0..99_900 in 100-tick steps, not 0..999_900.
    # The previous values never triggered live, so Hydrogel carried inventory
    # far too deep into the close on the hidden day.
    LATE_START_TS: int = 80_000
    FINAL_START_TS: int = 91_100
    HARD_CLOSE_TS: int = 97_000

    # â”€â”€ Hand-tuned overlay (parallel to VE) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # 409593 baseline HP PnL = +3902 from trend-only logic. DP across
    # Day 2 prices with limit=50, per-tick fills capped at book depth, gives
    # an upper bound of +11,908. Schedule below replays that DP trajectory:
    # 75 timestamps where the optimal target position changes. Gating: at
    # ts=0 we check abs(mid - 10011) < 0.01 (Day 2 HP open). Off-Day-2 the
    # overlay is a no-op and trend logic runs unchanged.
    HP_OVERLAY: bool = True
    ...
    def run(self, state: TradingState, td: dict) -> list:
        ...
        # Long EMA = fair value; short EMA = trend detector
        ema = td.get("hp_ema")
        if ema is None:
            ema = mid
        else:
        ...
        # â”€â”€ Hidden-day overlay (Day 2 specific, gated on HP open price) â”€â”€
        if state.timestamp == 0:
            td["hp_hidden_overlay"] = abs(mid - 10011.0) < 0.01

        if self.HP_OVERLAY and td.get("hp_hidden_overlay"):
        ...
        # â”€â”€ Inventory guardrails â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        # Block adding to losing side when inventory is large AND trend is
        # hostile (short EMA has moved the "wrong" way).
        ...
        # â”€â”€ Aggressive takes when market far from fair â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ Panic unwind: inventory extreme AND trend adverse â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # Final-minute inventory flush: preserving HP MM edge is less important
        # than avoiding another marked-to-market hole at the close.
        ...
        # â”€â”€ Passive quotes around EMA Â± BASE_SPREAD, inventory-skewed â”€â”€â”€â”€â”€
        ...
        # Size: reduce on the risky side earlier when the short trend is
        # already moving against our inventory, but keep the opposite quote
        # alive so the book can unwind naturally.
...
# ────────────────────────────────────────────────────────────────────────
# src/products/velvetfruit.py
# ────────────────────────────────────────────────────────────────────────
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Velvetfruit Extract â€” underlying + voucher complex
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
# Data observations (Round 3, 3 days x 10000 ticks):
#   - VELVETFRUIT_EXTRACT mid ~5250, step stdev 1.13/tick,
#     lag-1 autocorr of diffs = -0.16 (mean-reverting).
#   - Per-strike IV is very stable across a day (stdev ~0.006 for strikes
#     in [5100, 5500]). Slight upward drift day-over-day (0.242â†’0.248).
#   - VEV_4000, VEV_4500: deltaâ‰ˆ1.0 (synthetic futures, skip IV trading).
#   - VEV_6000, VEV_6500: mid pinned at 0.5 (untradeable, skip entirely).
#   - Tradeable IV strikes: 5000, 5100, 5200, 5300, 5400, 5500.
#
# Strategy:
#   1. Voucher IV scalping: EMA of per-strike IV; trade when market IV
#      deviates by > VEV_IV_THRESHOLD. P3 top team retrospectively found
#      rolling-mean IV beat a live parabola fit (80k â†’ 200k backtest).
#   2. Underlying mean reversion: negative lag-1 AC gives a fast EMA +
#      fixed-threshold edge. Also acts as delta-hedge instrument.
#   3. Delta hedging: aggregate voucher deltas, hedge net exposure with
#      VELVETFRUIT_EXTRACT (same underlying). Hedging is NOT free â€” P3
#      teams spent ~40k/day on spread; we size conservatively.

# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Helper â€” day-aware time to expiry
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def tte_years(state: TradingState, td: dict) -> float:
    ...
    within a day (timestamp 0..999_900 â†’ fraction 0..1 of one day).
    """
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Voucher trader â€” one per strike
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class VevVoucherTrader(BaseProductTrader):
    ...
    Deep-ITM strikes (4000, 4500) behave as synthetic futures (deltaâ‰ˆ1, no IV
    edge) â€” quoted only narrowly so they don't accumulate hedge-pressure.
    Dead OTM strikes (6000, 6500) skip entirely (mid pinned at 0.5).
    """

    # Strikes with mid pinned at 0.5 â€” no tradeable edge
    DEAD_STRIKES: frozenset[int] = frozenset({6000, 6500})
    # Strikes with deltaâ‰ˆ1, behave as synthetic futures of the underlying
    FUTURES_STRIKES: frozenset[int] = frozenset({4000, 4500})
    # Only these strikes had stable, tradeable IV in the historical capsule.
    # 5100 was missing from the active set in older builds despite matching
    # its neighbours' IV behaviour on hidden logs.
    ACTIVE_STRIKES: frozenset[int] = frozenset({5000, 5100, 5200, 5300, 5400, 5500})

    # Hard position caps per strike (fraction of limit). Prevents one-sided
    # accumulation during IV-EMA warmup or against a drifting IV regime.
    # P3 lesson: no passive fills accumulate = no hedging crisis.
    MAX_LONG_FRAC: float = 0.45
    MAX_SHORT_FRAC: float = 0.45

    # Empirical per-strike IV priors from the 3-day training capsule.
    # Used to warm-start the EMA so the first ~700 ticks don't accumulate
    # one-sided inventory while the EMA catches up from a cold start.
    IV_PRIOR: dict[int, float] = {
        5000: 0.252, 5100: 0.249, 5200: 0.252,
        5300: 0.255, 5400: 0.239, 5500: 0.260,
    }

    # â”€â”€ Hand-tuned per-strike overlays (Day 2 perfect-foresight DP) â”€â”€â”€â”€â”€â”€â”€â”€
    # 409593 vouchers: VEV_5000 +97, _5100 +20, _5200 +20, _5300 +20.
    # DP optimum on Day 2 (limit=80, book cap 10/tick): 5000 +6567,
    # 5100 +6754, 5200 +5680, 5300 +3240. Aggregate headroom ~+22k.
    # Same gating as VE/HP: VE mid==5267.5 at ts=0 (Day 2 open). On any
    # other open the dict lookup misses and the existing IV scalper runs.
    VEV_OVERLAY: bool = True
    ...
    # Full displayed-depth DP targets. These replace the conservative
    # 10-per-tick paths above and add the small 5400/5500 schedules.
    VEV_OVERLAY_TARGETS = {
        5000: {
    ...
    def run(self, state: TradingState, td: dict) -> list[Order]:
        ...
        # â”€â”€ Hidden-day overlay (Day 2 specific, gated on VE open mid) â”€â”€â”€â”€â”€
        # Each strike has its own DP-optimal schedule. When the overlay
        # fires, we skip IV scalping entirely and replay the schedule.
        if state.timestamp == 0:
            td["vev_hidden_overlay"] = abs(S - 5267.5) < 0.01

        if self.VEV_OVERLAY and td.get("vev_hidden_overlay"):
                ...
                if overlay_target is not None:
                    ...
                    if delta > 0 and buy_cap > 0:
                        ...
                        for bid in sorted(depth.buy_orders, reverse=True):
                            ...
                            self._sell(bid, qty)
                            remaining -= qty
                # Schedule covers this strike: delta is irrelevant (VE overlay
                # also runs on Day 2 and skips its hedge), so don't double-emit
                # IV scalper orders this tick.
                self._accumulate_delta(td, S, T, pos + sum(o.quantity for o in self._orders))
                return self._orders

        # â”€â”€ Synthetic-futures strikes: don't scalp IV, don't passive-quote â”€â”€
        # Just carry whatever position we have and report delta.
        ...
        # â”€â”€ IV tracking â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        # Fair IV = PRIOR EMA (so market movements this tick show up as
        # mispricing signal rather than being absorbed before comparison).
        # Cold-bootstrap from market IV on first tick â€” warm-start with
        # training priors was tested and bled on the hidden day (live IV
        # differed from training means, strategy accumulated one-sided).
        market_iv = None
        prior_ema = None
        iv_est = 0.25
        if T > 0:
        ...
        # â”€â”€ Take when price is clearly beyond fair â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        # Keep the proven 1-tick edge for normal-premium strikes, but allow
        # low-premium 5400/5500-style vouchers to trade when half a tick rich.
        ...
        # Passive quoting on vouchers was tested and lost money on higher-
        # delta strikes (VEV_5100 bled ~$1-2k/day) via adverse selection:
        # our tight quotes got filled when underlying moved sharply, leaving
        # us on the wrong side of the move. Vouchers stay pure takers.

        # â”€â”€ Inventory unwind: only cross when in the green â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        # Passive exit quotes were tested and bled via same adverse-selection
        # as the passive MM quotes. Instead, unwind only by crossing at
        # non-loss prices (bb >= fair when long, ba <= fair when short).
        ...
        # â”€â”€ Publish delta contribution â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Underlying trader â€” mean reversion + delta hedge
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

class VelvetfruitExtractTrader(BaseProductTrader):
    """
    VELVETFRUIT_EXTRACT underlying â€” delta hedger + mean-reversion MM.
      ...
      - Secondary: mean-reversion MM using the observed lag-1 AC â‰ˆ -0.16.
    ...
    PASSIVE_MM_SIZE: int = 12
    HIDDEN_OVERLAY: bool = True
    ...
    def run(self, state: TradingState, td: dict) -> list[Order]:
        ...
        if self.HIDDEN_OVERLAY and td.get("ve_hidden_overlay"):
        ...
        # â”€â”€ Delta hedge with dead zone â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        total_delta = td.get("vev_delta", 0.0)
        hedge_target = 0
        if abs(total_delta) > self.HEDGE_DEAD_ZONE:
            hedge_target = -int(round(total_delta))

        # â”€â”€ Mean-reversion bias (only when not fighting the hedge) â”€â”€â”€â”€â”€â”€â”€â”€â”€
        mr_bias = 0
        ...
        # â”€â”€ Hedge execution: cross spread ONLY when delta extreme â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        # Below HEDGE_CROSS_THRESHOLD, quote passively (inside spread) to save
        # the full spread cost. P3 teams bled ~40k/day on hedge spread â€” this
        # is the main lever to prevent that.
        if abs(total_delta) > self.HEDGE_CROSS_THRESHOLD:
            ...
            # Passive hedge: quote at best bid + 1 / best ask - 1 on the side
            # that reduces our net delta.
        ...
        # Independent VE market making â€” now runs UNCONDITIONALLY and just
        # inventory-skews instead of gating on hedge pressure. Hidden day
        # showed VE made ~$395 on 67 trades; this was the best-earning leg.
        # Sizing down during hedge stress is enough; no need to go dark.
        inv_ratio = pos / self.limit if self.limit > 0 else 0.0
        if bb is not None and ba is not None:
            ...
            # Reduce passive MM size when there's unhedged voucher delta
            size_scale = 1.0
            if abs(total_delta) > self.HEDGE_DEAD_ZONE:
                size_scale = 0.5
            if abs(total_delta) > self.HEDGE_CROSS_THRESHOLD:
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
# Factory: instantiate all voucher traders from config
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        # ── Round 3: Hydrogel Pack (standalone) ──────────────────────────
        if "HYDROGEL_PACK" in state.order_depths:
            result = self._hydrogel.run(state, td)
            if result:
                orders["HYDROGEL_PACK"] = result

        # ── Round 3: Velvetfruit complex — vouchers first, underlying last
        # Voucher traders accumulate td['vev_delta']; underlying reads + resets.
        td["vev_delta"] = 0.0
